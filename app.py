...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS admins (username TEXT PRIMARY KEY, password TEXT);
    ...
    # Ensure admin credentials are correctly inserted or updated
    cursor.execute("INSERT OR REPLACE INTO admins (username, password) VALUES (?, ?)", ("admin", "password123"))
...
def add_new_user():
    ...
    if request.method == 'POST':
        ...
        if role == 'student':
            ...
            cursor.execute("INSERT INTO users (id, name, role, username, password, course, section, year) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", 
                           (student_id, name, role, username, password, course, section, year))

        elif role == 'teacher':
            teacher_id = request.form['teacher_id']
            name = request.form['name_teacher']
            course = request.form['course_teacher']
            section = request.form['section_teacher']
            year = request.form['year_teacher']
            cursor.execute("INSERT INTO users (id, name, role, username, password, course, section, year) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", 
                           (teacher_id, name, role, username, password, course, section, year))
